...
char *command_path(char *string)
{
	...
	if (_strchr(string, '/') && stat(string, &st) == 0) /* check if input is an absolute path */
		return (string);
	...
	{
		cmd_path = malloc((sizeof(char) * _strlen(token)) + (sizeof(char) * _strlen(string)) + 2); /* allocate memory */
		if (!cmd_path) /* handle malloc error */
			break;
		sprintf(cmd_path, "%s/%s", token, string); /* concatenate command with its path */
